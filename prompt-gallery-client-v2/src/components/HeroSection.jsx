      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="flex flex-col items-start lg:col-span-6">
            {/* Main Headline */}
            <h1 className="font-display text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-6xl lg:text-7xl lg:leading-[1.08]">
            ...
            {/* Subtitle */}
            <p className="mt-6 max-w-2xl text-base font-normal leading-relaxed text-slate-600 dark:text-slate-300 sm:text-lg">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4">
              ...
              <Link
                to="/libraries"
                className="glass-card inline-flex items-center gap-2 rounded-full px-7 py-3.5 text-xs font-extrabold uppercase tracking-wider text-slate-800 dark:text-white hover:bg-white/60 dark:hover:bg-slate-800/80 transition-all"
              >
          ...
          <div className="lg:col-span-6 pt-6 lg:pt-0">
            <div className="relative min-h-[460px] sm:min-h-[500px] flex flex-col items-center justify-center perspective-1000">
              ...
              <div className="relative w-full flex items-center justify-center min-h-[420px]">
                {FEATURED_MODULES.map((item, index) => {
                  ...
                  if (isLeft) {
                    transformClass = '-translate-x-12 sm:-translate-x-20 scale-90 opacity-60 z-10 -rotate-y-12 shadow-lg hover:opacity-80'
                  } else if (isRight) {
                    transformClass = 'translate-x-12 sm:translate-x-20 scale-90 opacity-60 z-10 rotate-y-12 shadow-lg hover:opacity-80'
                  }
                  ...
                  return (
                    <div
                      ...
                      className={`absolute w-full max-w-sm sm:max-w-md transition-all duration-500 ease-out cursor-pointer transform-style-3d ${transformClass}`}
                    >
                      <div className="glass-card rounded-3xl overflow-hidden border border-white/80 dark:border-slate-800 p-5 space-y-3.5 shadow-2xl">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="rounded-full bg-slate-900/5 dark:bg-white/10 px-2.5 py-0.5 text-[10px] font-extrabold text-slate-700 dark:text-slate-200">
                            {item.model}
                          </span>
                        </div>

                        {/* Image Preview Container */}
                        <div className="relative h-44 sm:h-48 rounded-2xl overflow-hidden group">
                          <img
                            src={item.image}
                            alt={item.title}
                          ...
                          <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white">
                            <div>
                              ...
                              <h3 className="font-display text-base font-extrabold mt-1">{item.title}</h3>
                            </div>

                            <button
                              type="button"
                              onClick={(e) => handleCopy(e, item)}
                              className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/20 backdrop-blur-md text-white hover:bg-orange-500 transition-colors shadow-lg"
                            >
                        ...
                        {/* Prompt text snippet */}
                        <p className="text-[11px] font-mono text-slate-600 dark:text-slate-300 line-clamp-2 bg-slate-100/80 dark:bg-slate-900/60 p-2.5 rounded-xl border border-slate-200/50 dark:border-slate-800">
                          "{item.prompt}"
                        </p>
                        ...
                        <div className="flex items-center justify-between pt-1 text-xs font-bold text-slate-500">
                          <div className="flex items-center gap-3 text-[11px]">
                            <span className="flex items-center gap-1">
                              <Heart size={13} className="text-rose-500 fill-rose-500" /> {item.likes}
                            </span>
                            <span className="flex items-center gap-1">
                              <Eye size={13} /> {item.copies} copies
                            </span>
                          </div>
